"""
Compute TCC (Temporal Correlation Coefficient) maps for CAS-Canglong V3.5.

This version reads the pre-generated evaluation dataset
`Infer/eval/model_v3.nc` and does not rerun model inference.
...
Output:
    - Infer/eval/tcc_v3.nc

Usage:
    cd /home/lhwang/Desktop/CanglongPhysics
    /home/lhwang/anaconda3/envs/torch/bin/python Infer/compute_tcc_v3.py
"""
...
def main():
    ...
    if not os.path.exists(EVAL_PATH):
        raise FileNotFoundError(
            ...
            'Run Infer/gen_eval_v3.py first to generate Infer/eval/model_v3.nc.'
        )
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            'model_name': 'CAS-Canglong V3.5',
            'model_version': 'V3.5 (CanglongV2_5)',
            'test_period': '2017-2021',
            'eval_source': 'Infer/eval/model_v3.nc',
    ...
    print('\nGlobal mean TCC (V3.5, from model_v3.nc):')
    print(f'{"Lead":>6} | {"TP":>8} {"T2M":>8} {"OLR":>8} {"Z500":>8}')
    print('-' * 46)
    for lead_idx in range(N_LEADS):
        vals = [np.nanmean(tcc[vi, lead_idx]) for vi in range(len(VAR_NAMES))]
        print(f'  {lead_idx+1:>4} | {vals[0]:>8.4f} {vals[1]:>8.4f} {vals[2]:>8.4f} {vals[3]:>8.4f}')
