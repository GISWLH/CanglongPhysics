...
def calculate_hydrostatic_balance_loss(output_upper_normalized, output_surface_normalized,
    ...
    基于气压高度公式：p_sfc = p_msl * exp(-g*z / (R_d * T_v))

    其中：
    - p_sfc: 表面气压 (sp, 索引19)
    - p_msl: 海平面气压 (msl, 索引20)
    - g: 重力加速度 (9.80665 m/s²)
    - z: 地表高程 (DEM)
    - R_d: 干空气气体常数 (287 J/(kg·K))
    - T_v: 虚温，近似使用2m温度 (t2m, 索引10)
    """
    ...
    masks = get_cached_masks(device)
    dem = masks['dem']  # 地表高程 (721, 1440)
    land_mask = masks['land_mask']  # 只在陆地上计算
    ...
    # 提取变量
    sp_model = output_surface_physical[:, idx_sp, 0, :, :]    # 模型预测的表面气压
    msl = output_surface_physical[:, idx_msl, 0, :, :]        # 海平面气压
    t2m = output_surface_physical[:, idx_t2m, 0, :, :]        # 2米温度

    # 使用虚温公式：T_v ≈ T * (1 + 0.61 * q)，简化为直接使用t2m
    # 因为近地面比湿影响较小
    T_v = t2m

    # 计算物理公式预测的表面气压
    # p_sfc = p_msl * exp(-g*z / (R_d * T_v))
...
print("=" * 60)
print("Training with FIXED Weights + Wind-Aware Double Shift")
print("=" * 60)
...
wind_direction_counts = {i: 0 for i in range(9)}  # 统计风向分布
...
for epoch in range(num_epochs):
    ...
    for input_surface_batch, input_upper_air_batch, target_surface_batch, target_upper_air_batch in train_pbar:
        ...
        # 前向传播（获取风向信息用于监控）
        # 使用未标准化的数据计算风向
        result = model(input_surface_norm, input_upper_air_norm, return_wind_info=True)
        output_surface, output_upper_air, wind_dir_id = result

        # 统计风向分布
        from canglong.wind_aware_shift import get_dominant_direction, get_regional_dominant_directions
        dominant_id = get_dominant_direction(wind_dir_id)
        wind_direction_counts[dominant_id] += 1

        # 获取分区域风向统计（仅第一个batch打印详细信息）
        if sum(wind_direction_counts.values()) == 1:
            regional_dirs = get_regional_dominant_directions(wind_dir_id, num_regions=(4, 8))
            print(f"\n  [Regional Wind Directions (4x8=32 regions)]:")
            print(f"  Shape: {regional_dirs.shape}")
            print(f"  First sample (4 lat x 8 lon):")
            print(f"  {regional_dirs[0].cpu().numpy()}")
            print(f"  Direction ID: 0=None, 1=N, 2=NE, 3=E, 4=SE, 5=S, 6=SW, 7=W, 8=NW\n")
    ...
    for dir_id, count in wind_direction_counts.items():
        if count > 0:
            pct = 100.0 * count / total_samples
            print(f"    {WIND_DIR_NAMES[dir_id]:4s}: {count:3d} ({pct:5.1f}%)")
