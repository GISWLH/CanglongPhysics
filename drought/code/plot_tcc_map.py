"""
Plot TCC map from saved data
Usage: python plot_tcc_map.py --lead 1
"""
import numpy as np
...
def plot_tcc_map(lead_time):
    """
    Plot TCC map for a specific lead time
    ...
    """
    print(f"="*70)
    print(f"Plotting TCC map for Lead {lead_time}")
    print(f"="*70)
    ...
    print(f"   - Plotting TCC map with geographic projection...")
    ...
    ax.set_title(f'Method 1: Gridpoint TCC Map (Lead {lead_time})\n' +
    ...
    # Save figure
    output_png = f'figures/tcc_map_lead{lead_time}.png'
    output_svg = f'figures/tcc_map_lead{lead_time}.svg'
    ...
    print("\n[OK] TCC map plotting completed successfully!")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot TCC map from saved data')
    parser.add_argument('--lead', type=int, required=True, choices=[1,2,3,4,5,6],
                        help='Lead time (1-6)')

    args = parser.parse_args()

    plot_tcc_map(args.lead)