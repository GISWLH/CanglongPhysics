"""
Plot temperature time series and scatter plot from saved data
Usage: python plot_temperature_line_scatter.py --lead 1
"""
import numpy as np
...
def plot_temperature_line_scatter(lead_time):
    """
    Plot temperature time series and scatter plot for a specific lead time
    ...
    """
    print(f"="*70)
    print(f"Plotting temperature time series and scatter for Lead {lead_time}")
    print(f"="*70)
    ...
    print(f"   - Plotting temperature time series...")
    ax1 = axes[0]
    ...
    ax1.set_title(f'Method 2: Global Mean Temperature Time Series (Lead {lead_time})\n' +
    ...
    print(f"   - Plotting temperature scatter plot...")
    ax2 = axes[1]
    ...
    ax2.set_title(f'Method 2: Temperature Scatter Plot (Lead {lead_time})\n' +
                  '(Shows correlation strength)',
                  fontweight='bold', fontsize=11)
    ax2.set_xlabel('Observed Temperature Anomaly (°C)', fontsize=10)
    ax2.set_ylabel('Forecast Temperature Anomaly (°C)', fontsize=10)
    ...
    # Overall title
    fig.suptitle(f'Temperature Time Series and Scatter Plot: CAS-Canglong Lead {lead_time} Week (2022-2023)',
                 fontsize=14, fontweight='bold', y=1.00)

    plt.tight_layout()

    # Save figure
    output_png = f'figures/temperature_tcc_line_scatter_lead{lead_time}.png'
    output_svg = f'figures/temperature_tcc_line_scatter_lead{lead_time}.svg'
    ...
    print("\n[OK] Temperature time series and scatter plotting completed successfully!")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot temperature time series and scatter from saved data')
    parser.add_argument('--lead', type=int, required=True, choices=[1,2,3,4,5,6],
                        help='Lead time (1-6)')

    args = parser.parse_args()

    plot_temperature_line_scatter(args.lead)