...
##    ax.set_title("At 1.5°C global warming", fontsize=9, pad=4)
    ...
    **kwargs : keyword arguments
    ...
    if colorbar:
        ...
        if label is not None:
            cbar.set_label(label, labelpad=1, size=8, loc='right')
        cbar.ax.tick_params(labelsize=9)
...
def add_boundary(ax, shp,  **kwargs):
    '''
    Plot the Chinese province map shapefile.
...
def add_sta(ax, da, xlim, lat, color='#1f77b4', **kwargs):
    '''
    Plot the Chinese province map shapefile.
    ...
    **kwargs
    ...
    ax.plot(MED, ydata, c=color, linewidth=0.75, label='Median', zorder=10)
    ax.fill_betweenx(ydata, MED + STD, MED - STD, alpha=0.12, linewidth=0, color=color,
