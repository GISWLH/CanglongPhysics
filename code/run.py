demo_start_time = '2025-11-12'
demo_end_time = '2025-11-25'
data_inner_steps = 24
import torch
import numpy as np
...
# 导入所有需要的类
from canglong import (
    Canglong, Encoder, Decoder, BasicLayer,
    EarthSpecificBlock, EarthAttention3D,
    UpSample, DownSample, Mlp,
    GroupNorm, Swish, ResidualBlock,
    UpSampleBlock, DownSampleBlock, NonLocalBlock
)

# 关键步骤：将这些类注册到 __main__ 模块
import __main__
__main__.Canglong = Canglong
__main__.Encoder = Encoder
__main__.Decoder = Decoder
__main__.BasicLayer = BasicLayer
__main__.EarthSpecificBlock = EarthSpecificBlock
__main__.EarthAttention3D = EarthAttention3D
__main__.UpSample = UpSample
__main__.DownSample = DownSample
__main__.Mlp = Mlp
__main__.GroupNorm = GroupNorm
__main__.Swish = Swish
__main__.ResidualBlock = ResidualBlock
__main__.UpSampleBlock = UpSampleBlock
__main__.DownSampleBlock = DownSampleBlock
__main__.NonLocalBlock = NonLocalBlock
...
def get_week_of_year(date):
    """
    计算一年中的周数，第一周为1-7日，最多52周。
    注意：12月31日(闰年还有12月30日)不计入任何周，这里将其映射到第52周。
    跨年时，新年的日期会正确返回1、2等周号。
    """
    day_of_year = date.dt.dayofyear
    week_num = ((day_of_year - 1) // 7) + 1
    # 将超过52周的情况（如12月31日，dayofyear=365，周数=53）映射到第52周
    if hasattr(week_num, 'clip'):
        return week_num.clip(max=52)
    else:
        return np.clip(week_num, 1, 52)
...
for i in range(start_pred_idx, len(D_pred.time)):
    ...
    for year in hist_years:
        ...
        if len(week_indices) > 0:
            ...
            # 确保有足够的前置周
            if week_idx >= 3:  # 当前年有足够的前置周
                # 累积当前周和前3周的D值
                accum_D = 0
                for j in range(4):
                    accum_D += year_data.isel(time=week_idx-j)
                hist_4week_accum_list.append(accum_D)
            elif week_idx < 3 and year > hist_years.min():
                # 跨年情况：当前周在年初，索引<3，需要从上一年借数据
                # 这包括周1、2、3（索引0、1、2）的情况
                prev_year = year - 1
                if prev_year in hist_years:
                    prev_year_data = D_hist.where(D_hist.time.dt.year == prev_year, drop=True)
                    # 累积D值：从当前周向前累积4周
                    accum_D = 0
                    weeks_needed = 4
                    # 先从当前年取数据
                    for j in range(week_idx + 1):  # 当前年能取到的周数
                        accum_D += year_data.isel(time=week_idx-j)
                        weeks_needed -= 1
                    # 再从上一年末尾取剩余的周
                    if weeks_needed > 0 and len(prev_year_data.time) >= weeks_needed:
                        for j in range(weeks_needed):
                            accum_D += prev_year_data.isel(time=-(j+1))
                        hist_4week_accum_list.append(accum_D)
