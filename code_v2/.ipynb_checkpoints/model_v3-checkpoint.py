...
class PhysicalConstraints(nn.Module):
    ...
    def hydrostatic_balance_loss(self, output_upper_air):
        ...
        delta_phi_physical = self.R_d * temp_avg * torch.log(torch.tensor(850.0/700.0))
...
class CanglongV3(nn.Module):
    ...
    def __init__(self, embed_dim=96, num_heads=(8, 16, 16, 8), window_size=(2, 6, 12),
                 ...
                 lambda_water=0.1, lambda_energy=0.1, lambda_pressure=0.1):
        super().__init__()
...
# Training utilities
def train_with_physical_constraints(model, train_loader, valid_loader, 
                                   surface_mean, surface_std, upper_mean, upper_std,
                                   num_epochs=50, learning_rate=0.0005, device='cuda'):
    """
    Training function with physical constraints
    """
    ...
    model.physical_constraints = PhysicalConstraints(
    ...
    for epoch in range(num_epochs):
        ...
        for input_surface, input_upper_air, target_surface, target_upper_air in train_pbar:
            ...
            if hasattr(model, 'module'):  # DataParallel case
                output_surface, output_upper_air, losses = model.module(
            ...
            train_pbar.set_postfix({
                ...
                "mse_s": f"{losses.get('mse_surface', 0).item():.4f}",
                "mse_u": f"{losses.get('mse_upper_air', 0).item():.4f}",
                "water": f"{losses.get('water_balance', 0).item():.4f}",
                "energy": f"{losses.get('energy_balance', 0).item():.4f}",
                "hydro": f"{losses.get('hydrostatic_balance', 0).item():.4f}"
            })
        ...
        with torch.no_grad():
            ...
            for input_surface, input_upper_air, target_surface, target_upper_air in valid_pbar:
                ...
                for key in valid_losses:
                    if key in losses:
                        valid_losses[key] += losses[key].item()
        ...
        for key in valid_losses:
            valid_losses[key] /= len(valid_loader)
        
        # Print epoch summary
        print(f"\nEpoch {epoch+1}/{num_epochs} Summary:")
        print(f"  Train - Total: {train_losses['total']:.6f}, MSE Surface: {train_losses['mse_surface']:.6f}, "
              f"MSE Upper: {train_losses['mse_upper_air']:.6f}")
        print(f"         Physical - Water: {train_losses['water_balance']:.6f}, Energy: {train_losses['energy_balance']:.6f}, "
              f"Hydrostatic: {train_losses['hydrostatic_balance']:.6f}")
        print(f"  Valid - Total: {valid_losses['total']:.6f}, MSE Surface: {valid_losses['mse_surface']:.6f}, "
              f"MSE Upper: {valid_losses['mse_upper_air']:.6f}")
        print(f"         Physical - Water: {valid_losses['water_balance']:.6f}, Energy: {valid_losses['energy_balance']:.6f}, "
              f"Hydrostatic: {valid_losses['hydrostatic_balance']:.6f}")
...
# 创建模型
model = CanglongV3()
train_with_physical_constraints(model, train_loader, valid_loader, torch.tensor(surface_mean), torch.tensor(surface_std), torch.tensor(upper_mean), torch.tensor(upper_std),num_epochs=50, learning_rate=0.0005, device='cuda')