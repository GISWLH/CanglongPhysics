...
def plot_closure_rates(data, save_dir):
    ...
    for i, (key, title) in enumerate(metrics):
        ...
        if key in obs_closure:
            ...
            # 转换观测数据为百分比
            plot_interval_with_caps(ax, x_pos, obs['mean'] * 100, obs['std'] * 100, '#2ca02c')

        ax.set_ylabel('Physical Consistency (Closure Rate)')
        ax.set_title(title)
        ...
        if key in ['valid_closure_temperature', 'valid_closure_momentum']:
            # 温度和动量方程保留2位小数
        ...
        # 设置x轴刻度标签（简短版本）
        short_labels = {
            'Base (Swin Core)': 'Swin Core',
            'Baseline (Wind Core)': 'Wind Core',
            'Water Balance': 'Water',
            'Energy Balance': 'Energy',
            'Hydrostatic Balance': 'Hydrostatic',
            'Local Temperature Tendency': 'Temperature',
            'Navier-Stokes': 'N-S',
            'Full Physical': 'Full',
        }
        x_labels = [short_labels.get(lbl, lbl) for lbl in box_labels]
        ax.set_xticks(range(1, len(box_labels) + 1))
        ax.set_xticklabels(x_labels, rotation=45, ha='right', rotation_mode='anchor', fontsize=8, va='top')
