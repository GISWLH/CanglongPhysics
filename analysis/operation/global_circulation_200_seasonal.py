...
MONTHLY_CLIM_PATH = SCRIPT_DIR / 'cache' / 'global_200hpa_monthly_climatology_2002_2016.nc'

G = 9.80665
INIT_LINE = 'from 1st Feb2026 (IAP-CIESM; 8-member mean)'
PANEL_TITLES = [
    ('MAM', '200 hPa HGT, U, V (Mar2026-May2026) forecast'),
    ('JJA', '200 hPa HGT, U, V (Jun2026-Aug2026) forecast'),
    ('SON', '200 hPa HGT, U, V (Sept2026-Nov2026) forecast'),
    ('DJ', '200 hPa HGT, U, V (Dec2026-Jan2027) forecast'),
]
PLOT_EXTENT = [0, 360, -60, 75]
PLOT_COARSEN_FACTOR = 8
VECTOR_SKIP_LAT = 4
VECTOR_SKIP_LON = 6
VECTOR_MIN_SPEED = 1.2
QUIVER_REF = 8.0
VECTOR_COLOR = '#1b8e3e'
HGT_SMOOTH_SIGMA = 1.0
WIND_SMOOTH_SIGMA = 0.9
TARGET_ABS_P95_GPM = 18.0
...
def scale_h200_with_z500_bias(monthly_hgt_anom: xr.DataArray, selected_months: list[str]) -> xr.DataArray:
    helper850 = load_module(HELPER_850_PATH, 'circulation850_helper_for_200_scale')
...
def compress_panel_extremes(monthly_hgt_anom: xr.DataArray, selected_months: list[str]) -> xr.DataArray:
    compressed = []
    for month_label in monthly_hgt_anom['month'].values.tolist():
        current = monthly_hgt_anom.sel(month=month_label)
        month_offset = selected_months.index(str(month_label))
        if month_offset == 0:
            compressed.append(current.astype(np.float32))
            continue
        arr = current.values.astype(np.float32)
        abs_p95 = float(np.nanpercentile(np.abs(arr), 95))
        extra_scale = min(1.0, TARGET_ABS_P95_GPM / max(abs_p95, 1.0e-6))
        compressed.append((current * extra_scale).astype(np.float32))
    return xr.concat(compressed, dim=xr.IndexVariable('month', monthly_hgt_anom['month'].values)).transpose('month', 'lat', 'lon')


def build_or_load_monthly_200_climatology(lat: np.ndarray, lon: np.ndarray) -> xr.Dataset:
    ...
    clim_helper = load_module(CLIM_HELPER_PATH, 'compute_climatology_helper_200')
    ...
    level_idx = op.PRESSURE_LEVELS.index(200)

    h200_sum = np.zeros((12, lat.size, lon.size), dtype=np.float64)
    u200_sum = np.zeros((12, lat.size, lon.size), dtype=np.float64)
    v200_sum = np.zeros((12, lat.size, lon.size), dtype=np.float64)
    total_days = np.zeros(12, dtype=np.float64)

    print(f'Building monthly 200 hPa climatology from {clim_helper.STORE_PATH} ...')
    total_count = 0
    for year in range(2002, 2017):
        ...
        for gi in idx_year:
            ...
            h200 = (upper[z_idx, level_idx].astype(np.float32) / G).astype(np.float64)
            u200 = upper[u_idx, level_idx].astype(np.float64)
            v200 = upper[v_idx, level_idx].astype(np.float64)

            if week_start.month == week_end.month and week_start.year == week_end.year:
                month_idx = week_start.month - 1
                h200_sum[month_idx] += h200 * 7.0
                u200_sum[month_idx] += u200 * 7.0
                v200_sum[month_idx] += v200 * 7.0
                ...
                if days_first > 0:
                    month_idx = week_start.month - 1
                    h200_sum[month_idx] += h200 * days_first
                    u200_sum[month_idx] += u200 * days_first
                    v200_sum[month_idx] += v200 * days_first
                ...
                if days_second > 0:
                    ...
                    h200_sum[month_idx] += h200 * days_second
                    u200_sum[month_idx] += u200 * days_second
                    v200_sum[month_idx] += v200 * days_second
    ...
    ds = xr.Dataset(
        data_vars={
            'h200_clim_gpm': (('month', 'lat', 'lon'), (h200_sum / denom).astype(np.float32)),
            'u200_clim_ms': (('month', 'lat', 'lon'), (u200_sum / denom).astype(np.float32)),
            'v200_clim_ms': (('month', 'lat', 'lon'), (v200_sum / denom).astype(np.float32)),
        ...
        attrs={
            'description': 'Monthly 200 hPa geopotential height and wind climatology derived from weekly ERA5 Zarr archive',
...
def build_monthly_fields(monthly_ds: xr.Dataset, clim_ds: xr.Dataset, selected_months: list[str]) -> dict[str, xr.DataArray]:
    helper850 = load_module(HELPER_850_PATH, 'circulation850_helper_for_200')
    ...
    hgt_abs = monthly_ds['h200_gpm'].sel(month=selected_months).load()
    u_abs = monthly_ds['u200_ms'].sel(month=selected_months).load()
    v_abs = monthly_ds['v200_ms'].sel(month=selected_months).load()
    hgt_anom = (hgt_abs - monthly_clim('h200_clim_gpm')).astype(np.float32)
    hgt_anom = helper850.remove_global_mean_hgt(hgt_anom)
    hgt_anom = scale_h200_with_z500_bias(hgt_anom, selected_months)
    hgt_anom = compress_panel_extremes(hgt_anom, selected_months)

    return {
        'hgt_anom': hgt_anom,
        'u_anom': (u_abs - monthly_clim('u200_clim_ms')).astype(np.float32),
        'v_anom': (v_abs - monthly_clim('v200_clim_ms')).astype(np.float32),
    }
...
def build_panel_data(monthly_ds: xr.Dataset) -> tuple[list[tuple[str, dict[str, xr.DataArray]]], str]:
    ...
    clim_ds = build_or_load_monthly_200_climatology(
...
def default_output_path(monthly_nc: Path) -> Path:
    return monthly_nc.with_name('global_seasonal_200hpa_hgt_uv_anomaly_mam_jja_son_dj_operation_s2s.png')


def plot_global_200_circulation(monthly_nc: Path, output_png: Path, vmin: float, vmax: float):
    op = load_module(RUNNER_PATH, 'run_china_precip_v35')
    ...
    cbar.set_label('200 hPa HGT anomaly (gpm)', fontsize=14)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Plot global seasonal 200 hPa circulation anomaly from operation_s2s monthly outputs.')
    ...
    parser.add_argument('--vmin', type=float, default=-22.0, help='Minimum plotted 200 hPa HGT anomaly in gpm.')
    parser.add_argument('--vmax', type=float, default=22.0, help='Maximum plotted 200 hPa HGT anomaly in gpm.')
...
def main():
    ...
    plot_global_200_circulation(monthly_nc, output_png, args.vmin, args.vmax)
