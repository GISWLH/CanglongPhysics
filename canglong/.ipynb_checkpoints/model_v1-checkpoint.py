...
A skillful 3D Transformer model for sub-seasonal to seasonal global weather prediction
"""
...
from .embed import ImageToPatch3D, ImageToPatch4D
from .recovery import RecoveryImage3D, RecoveryImage4D
from .pad import calculate_padding_3d
from .crop import center_crop_3d
...
class Canglong(nn.Module):
    ...
    This model takes surface and upper-air weather data as input and produces weather forecasts.

    Args:
        embed_dim (int): Base embedding dimension. Default: 96
        num_heads (tuple): Number of attention heads in different layers. Default: (8, 16, 16, 8)
        window_size (tuple): Window size for attention computation. Default: (2, 6, 12)
        load_earth_constant (bool): Whether to load Earth constant data. Default: True
        earth_constant_path (str): Path to Earth constant data file. Default: 'constant_masks/Earth.pt'

    Input:
        - surface: (batch, 26, time, lat, lon) - Surface variables
        - upper_air: (batch, 10, 5, time, lat, lon) - Upper air variables at 5 pressure levels

    Output:
        - output_surface: (batch, 26, 1, lat, lon) - Predicted surface variables
        - output_upper_air: (batch, 10, 5, 1, lat, lon) - Predicted upper air variables
    """
    ...
    def __init__(self,
                 ...
                 num_heads=(8, 16, 16, 8),
                 window_size=(2, 6, 12),
                 load_earth_constant=True,
                 earth_constant_path='constant_masks/Earth.pt'):
        super().__init__()
        ...
        # Encoder and decoder for surface data
        ...
        # Recovery layers
        self.patchrecovery3d = RecoveryImage3D(
            image_size=(26, 721, 1440),
            patch_size=(1, 4, 4),
        ...
        if load_earth_constant:
            try:
                import os
                if os.path.exists(earth_constant_path):
                    self.input_constant = torch.load(earth_constant_path, weights_only=True)
                    if torch.cuda.is_available():
                        self.input_constant = self.input_constant.cuda()
                else:
                    print(f"Warning: Earth constant file not found at {earth_constant_path}")
                    self.input_constant = None
            except Exception as e:
                print(f"Warning: Could not load Earth constant data: {e}")
                self.input_constant = None
        else:
            self.input_constant = None
    ...
    def forward(self, surface, upper_air):
        """
        Forward pass of the Canglong model.

        Args:
            surface: (batch, 26, time, lat, lon) - Surface variables
            upper_air: (batch, 10, 5, time, lat, lon) - Upper air variables
        ...
        # Process earth constant if available
        if self.input_constant is not None:
            constant = self.conv_constant(self.input_constant)
        else:
            # Create a dummy constant tensor if not available
            B = surface.shape[0]
            device = surface.device
            constant = torch.zeros(B, 96, 181, 360, device=device)

        # Encode surface and upper air data
        ...
        x = torch.concat([
            upper_air.squeeze(3),
            surface,
            constant.unsqueeze(2) if constant.dim() == 4 else constant
        ], dim=2)
        ...
        # Split output for surface and upper air
        output_surface = output[:, :, 3:5, :, :]  # Layers 4-5 are surface
        output_upper_air = output[:, :, 0:3, :, :]  # Layers 0-3 are upper air
