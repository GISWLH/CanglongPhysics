"""
CAS-Canglong V2.3 Training Script
...
from canglong import CanglongV2_3
...
if __name__ == '__main__':
    ...
    # V2.3 needs mean/std to denormalize u/v for physical wind direction computation
    model = CanglongV2_3(
    ...
    save_dir = 'checkpoints_v2_3'
    os.makedirs(save_dir, exist_ok=True)

    # Try to resume from existing checkpoint
    start_epoch = 0
    checkpoint_files = sorted(Path(save_dir).glob('model_v2_3_epoch*.pth'),
    ...
    print("=" * 70)
    print("Training V2.3 with Wind-Aware Per-Window Shift + Matching Attention Mask")
    print("=" * 70)
    print("Window size: (2, 6, 12)")
    print("Wind shift scale: 2")
    print("Wind speed threshold: 0.5 m/s")
    print("Top-K wind directions: 4")
    print("=" * 70)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            save_path = os.path.join(save_dir, f"model_v2_3_epoch{epoch+1}.pth")
            if hasattr(model, 'module'):
