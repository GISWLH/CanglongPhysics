"""
CAS-Canglong V2.1 Training Script
V2.1: Wind-aware global shift with per-window direction + matching attention mask
"""
...
from canglong import CanglongV2_1
...
if __name__ == '__main__':
    ...
    # V2.1 needs mean/std to denormalize u/v for physical wind direction computation
    model = CanglongV2_1(
    ...
    save_dir = 'checkpoints_v2_1'
    os.makedirs(save_dir, exist_ok=True)

    # Try to resume from existing checkpoint
    start_epoch = 0
    checkpoint_files = sorted(Path(save_dir).glob('model_v2_1_epoch*.pth'),
    ...
    print("=" * 70)
    print("Training V2.1 with Wind-Aware Per-Window Shift + Matching Attention Mask")
    print("=" * 70)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            checkpoint_path = f"{save_dir}/model_v2_1_epoch{epoch+1}.pth"
    ...
    # Save final model
    final_path = f"{save_dir}/model_v2_1_final.pth"
